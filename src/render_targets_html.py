...
def build_rows(year):
    ...
    for _,r in d.iterrows():
        ...
        rows.append(dict(name=r.PlayerName,team=r.Team if isinstance(r.Team,str) else '',age=f(r.Age),role=r.role,ip=f(r.IP),slot=f(r.arm_angle),eff4=f(r.eff4,2),
            ...
            gate=int(pd.notna(r.cpm)),cpm=f(r.cpm,2),dev=f(r.pred,1),zdev=f(r.z_dev,2),zact=f(r.z_act,2),ppstf=f(r.pp_stf),ppuse=f(r.pp_use*100) if pd.notna(r.pp_use) else None,ppfam=(r.pp_fam if isinstance(r.pp_fam,str) else ''),ppraw=f(r.pp_raw),fbliab=f(r.fb_liability,1),
...
function render(){const tb=document.getElementById('tb');const q=filt.q.toLowerCase();
 ...
 tb.innerHTML=rows.map((r,i)=>`<tr class="row" tabindex="0" data-i="${D.indexOf(r)}"><td class="l" style="color:var(--muted)">${i+1}</td>`+cols.map(([k,,c])=>`<td class="${c||''}">${k==='ppstf'&&r.ppstf!==null?`${r.ppfam} ${r.ppraw}→${r.ppstf} @ ${r.ppuse}%`:fmt(r[k],k)}${k==='slot'&&r.drop?'<span class="flag" title="Driveline drop recipe: below-avg IVB, eff≥.93"></span>':''}</td>`).join('')+'</tr>').join('');

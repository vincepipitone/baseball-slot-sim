...
def build_rows(year):
    ...
    cpm=pd.read_parquet('data/derived/cpm_2026.parquet' if year==2026 else 'data/derived/cpm_test2025.parquet')[['pitcher','z_dev','z_act','cpm','in_gate']]
    ...
    for _,r in d.iterrows():
        ...
        rows.append(dict(name=r.PlayerName,team=r.Team if isinstance(r.Team,str) else '',age=f(r.Age),role=r.role,ip=f(r.IP),slot=f(r.arm_angle),eff4=f(r.eff4,2),
            ...
            gate=int(r.in_gate) if pd.notna(r.in_gate) else 0,cpm=f(r.cpm,2),dev=f(r.pred_cal,1) if pd.notna(r.pred_cal) else None,devraw=f(r.pred,1),zdev=f(r.z_dev,2),zact=f(r.z_act,2),ppstf=f(r.pp_stf),ppuse=f(r.pp_use*100) if pd.notna(r.pp_use) else None,ppfam=(r.pp_fam if isinstance(r.pp_fam,str) else ''),ppraw=f(r.pp_raw),fbliab=f(r.fb_liability,1),

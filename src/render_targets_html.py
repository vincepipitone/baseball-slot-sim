...
def build_rows(year):
    ...
    cpm=pd.read_parquet('data/derived/cpm_2026.parquet' if year==2026 else 'data/derived/cpm_test2025.parquet')[['pitcher','pred','z_dev','z_act','cpm','pp_stf','pp_use','fb_liability']]
    d=d.merge(cpm,on='pitcher',how='left')
    ...
    for _,r in d.iterrows():
        ...
        rows.append(dict(name=r.PlayerName,team=r.Team if isinstance(r.Team,str) else '',age=f(r.Age),role=r.role,ip=f(r.IP),slot=f(r.arm_angle),eff4=f(r.eff4,2),
            cls=r.suppro_class if isinstance(r.suppro_class,str) else 'unknown',stuff=f(r.stuff),own=f(r.stuff_B_plus_oos),ownfg=f(r.stuff_B_plus_oos-r.stuff,1),loc=f(r.sp_location),pit=f(r.sp_pitching),
            gate=int(pd.notna(r.cpm)),cpm=f(r.cpm,2),dev=f(-0.29+0.69*r.pred,1) if pd.notna(r.pred) else None,devraw=f(r.pred,1),pgain=(f(min(0.9,max(0.05,0.10+0.075*(-0.29+0.69*r.pred))),2) if pd.notna(r.pred) else None),zdev=f(r.z_dev,2),zact=f(r.z_act,2),ppstf=f(r.pp_stf),ppuse=f(r.pp_use*100) if pd.notna(r.pp_use) else None,ppfam=(r.pp_fam if isinstance(r.pp_fam,str) else ''),ppraw=f(r.pp_raw),fbliab=f(r.fb_liability,1),
...
function fmt(v,k){if(v===null||v===undefined)return '<span style="color:var(--muted)">·</span>';if(k==='cls'){const g=CLSG[v]||'H';return `<span class="chip ${g}">${v.replace('_',' ')}</span>`}
 if(k==='gain'||k==='dwar'||k==='dm'||k==='gap'||k==='mix'||k==='mixb'||k==='coors'||k==='ownfg'){const s=(v>0?'+':'')+v.toFixed(k==='dwar'?2:1);return v>0?`<span class="pos">${s}</span>`:v<0?`<span class="neg">${s}</span>`:s}
...
function detail(r){const fams=r.fams.map(x=>`<div class="fam ${x.reach?'reach':'off'}"><div class="h">${x.fam} <span style="font-weight:400;color:var(--muted)">${FAMN[x.fam]}</span></div><div class="m">prec Stf+ ${x.pstf??'·'} · share ${x.prec??'·'} · P(add) ${x.p??'·'}</div><div class="m">gain ${x.gain>0?'+'+x.gain:'0'} · EV ${x.ev??0}</div></div>`).join('');
 return `<td colspan="${cols.length+1}"><div><strong>${r.name}</strong> — ${r.role}, ${r.ip} IP, ERA ${r.era??'·'} / xERA ${r.xera??'·'}, K% ${r.k??'·'} · BB% ${r.bb??'·'} · slot ${r.slot}°, 4S efficiency ${r.eff4}, ${r.cls.replace('_',' ')}, precedent pool ${r.pool} same-hand pitcher-seasons within ±5°/similar VAA${r.drop?' · <span style="color:var(--flag)">drop-recipe profile</span>':''}.<br>Families not thrown (&lt;2%). Outlined = reachable (≥20% of sup/pro-compatible comps throw it ≥10%). Gain = 0.14 × (precedent Stf+ − ${r.stuff})⁺; EV = P(add) × gain.</div><div class="fams">${fams||'<em>No unthrown families with precedent data.</em>'}</div><div style="margin-top:10px">Own-model Stuff ${r.own} vs FG ${r.stuff} (${r.ownfg>0?'+':''}${r.ownfg}). ${r.dev!==null?`<strong>Development engine</strong>: raw prediction ${r.devraw>0?'+':''}${r.devraw}, calibrated ${r.dev>0?'+':''}${r.dev} next-season Stuff+ (rolling backtests 2021–25: realized ≈ −0.3 + 0.69×raw; sd ≈ 6 in every bucket; +4–6 bucket realized +4.8 with 44% chance of a 5+ gain). `:''}<strong>Actionable ${r.act}</strong> = 0.5×mix ${r.mixb>0?'+'+r.mixb:'0'} (${r.mixplan||'no reallocation'}${r.merged?'; merged labels: '+r.merged:''}) + add ${r.addact>0?'+'+r.addact:'0'} + drop ${r.dropb>0?'+'+r.dropb:'0'} → projected Stuff+ ${r.projact}. <strong>Regress-to-comps</strong> ${r.gap>0?'+'+r.gap:'0'} — ${r.gapp||'no pitch grades below its comps'}. <strong>Mix</strong> ${r.mix>0?'+'+r.mix:r.mix} — worst pitch ${r.worst||'·'}, best ${r.bestp||'·'}; reweighting up to 20 pts of usage toward better pitches (FB ≥30%, cap 45%). <strong>Coors</strong> ${r.coors>0?'+'+r.coors:r.coors}${r.colshare?` (${Math.round(r.colshare*100)}% of pitches at Coors; own-model road minus all)`:''}. <strong>Drift</strong> ${r.opp} = .31×regress + 1.9×add-EV + .06×mix + .14×Coors (weights fit on 2020–25 next-season ΔStuff+).</div><div class="legend">Projected Stuff+ ${r.proj} → ΔPit+ ${r.dpit>0?'+':''}${r.dpit} → ΔWAR ${r.dwar>0?'+':''}${r.dwar} at ${r.ip} IP (${r.role} rate) → ${r.dm>0?'+':''}$${r.dm}M at $8M/WAR. Conditional on the add happening; not a forecast.</div></td>`}
...
setYear('2026');
"""
ths=''.join(f'<th data-k="{k}" class="{c if len(x)>2 else ""}">{lab}</th>' for x in [['name','Pitcher','l'],['team','Tm','l'],['age','Age'],['role','Role','l'],['ip','IP'],['slot','Slot°'],['eff4','4S eff'],['cls','Class','l'],['stuff','Stuff+'],['ownfg','Own−FG'],['loc','Loc+'],['act','Actionable'],['mixb','Mix'],['best','Best add','l'],['bpstf','Prec Stf+'],['addact','Add gain'],['dropb','Drop'],['projact','Proj Stf+'],['opp','Drift'],['gap','Gap vs comps → exp.'],['ev','Add ΣEV'],['bp','P(add)'],['coors','Coors'],['pool','Pool']] for k,lab,*c in [x] for c in [c[0] if c else ''])
def check(nm):

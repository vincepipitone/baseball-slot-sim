...
def build_rows(year):
    ...
    for _,r in d.iterrows():
        ...
        rows.append(dict(name=r.PlayerName,team=r.Team if isinstance(r.Team,str) else '',age=f(r.Age),role=r.role,ip=f(r.IP),slot=f(r.arm_angle),eff4=f(r.eff4,2),
            ...
            gate=int(pd.notna(r.cpm)),cpm=f(r.cpm,2),dev=f(r.pred,1),zdev=f(r.z_dev,2),zact=f(r.z_act,2),ppstf=f(r.pp_stf),ppuse=f(r.pp_use*100) if pd.notna(r.pp_use) else None,fbliab=f(r.fb_liability,1),
            ...
            mix=f(r.gain_mix,1),gap=f(r.gain_gap,1),gapp=(r.gap_parts if isinstance(r.gap_parts,str) else ''),coors=f(r.coors_adj,1),colshare=f(r.col_share,2) if pd.notna(r.col_share) else None,
...
const COLS26=[['name','Pitcher','l'],['team','Tm','l'],['age','Age'],['role','Role','l'],['ip','IP'],['slot','Slot°'],['eff4','4S eff'],['cls','Class','l'],['stuff','Stuff+'],['own','Own'],['loc','Loc+'],['pit','Pit+'],['gate','Gate'],['ppstf','Plus pitch'],['dev','Dev ΔStf+'],['act','Actionable'],['cpm','CPM score'],['mixb','Mix'],['best','Best add','l'],['bpstf','Prec Stf+'],['addact','Add gain'],['dropb','Drop'],['projact','Proj Stf+'],['opp','Drift'],['gap','Regress'],['pool','Pool']];const COLS25=COLS26.concat([['rstf','ΔStf+ next'],['rpit','ΔPit+ next']]);let cols=COLS26;
...
function render(){const tb=document.getElementById('tb');const q=filt.q.toLowerCase();
 ...
 tb.innerHTML=rows.map((r,i)=>`<tr class="row" tabindex="0" data-i="${D.indexOf(r)}"><td class="l" style="color:var(--muted)">${i+1}</td>`+cols.map(([k,,c])=>`<td class="${c||''}">${k==='ppstf'&&r.ppstf!==null?`${r.ppstf} @ ${r.ppuse}%`:fmt(r[k],k)}${k==='slot'&&r.drop?'<span class="flag" title="Driveline drop recipe: below-avg IVB, eff≥.93"></span>':''}</td>`).join('')+'</tr>').join('');
...
<div class="stats">
<div class="stat"><div class="v">#7 · #13</div><div class="l">Palmquist going into 2026 · Way today, on the CPM composite within the gate</div></div><div class="stat"><div class="v">+2.5 / +2.5</div><div class="l">2026 Stuff+ / Pitching+ excess of the as-of-2025 ACTIONABLE top-40 vs matched controls (n=23; top-80 +2.1 / +1.6)</div></div>

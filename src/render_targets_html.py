...
let sortKey='act',sortDir=-1,filt={q:'',role:'',cls:'',reach:false,drop:false};
const cols=[['name','Pitcher','l'],['team','Tm','l'],['age','Age'],['role','Role','l'],['ip','IP'],['slot','Slot°'],['eff4','4S eff'],['cls','Class','l'],['stuff','Stuff+'],['own','Own'],['loc','Loc+'],['act','Actionable'],['mix','Mix'],['best','Best add','l'],['bpstf','Prec Stf+'],['addact','Add gain'],['dropb','Drop'],['projact','Proj Stf+'],['opp','Drift'],['gap','Regress'],['ev','Add ΣEV'],['bp','P(add)'],['coors','Coors'],['pool','Pool']];
function fmt(v,k){if(v===null||v===undefined)return '<span style="color:var(--muted)">·</span>';if(k==='cls'){const g=CLSG[v]||'H';return `<span class="chip ${g}">${v.replace('_',' ')}</span>`}
 ...
 if(k==='act'){return `<span class="bar" style="width:${Math.min(70,v*6)}px"></span>${v.toFixed(1)}`}if(k==='opp')return v.toFixed(2);if(k==='dropb'||k==='addact')return v>0?`<span class="pos">+${v.toFixed(1)}</span>`:'0';if(k==='ev')return v.toFixed(2);
...
render();
"""
ths=''.join(f'<th data-k="{k}" class="{c if len(x)>2 else ""}">{lab}</th>' for x in [['name','Pitcher','l'],['team','Tm','l'],['age','Age'],['role','Role','l'],['ip','IP'],['slot','Slot°'],['eff4','4S eff'],['cls','Class','l'],['stuff','Stuff+'],['own','Own'],['loc','Loc+'],['act','Actionable'],['mix','Mix'],['best','Best add','l'],['bpstf','Prec Stf+'],['addact','Add gain'],['dropb','Drop'],['projact','Proj Stf+'],['opp','Drift'],['gap','Regress'],['ev','Add ΣEV'],['bp','P(add)'],['coors','Coors'],['pool','Pool']] for k,lab,*c in [x] for c in [c[0] if c else ''])
def check(nm):
...
<div class="stats">
<div class="stat"><div class="v">+3.0</div><div class="l">2026 Stuff+ excess of the as-of-2025 ACTIONABLE top-40 vs stuff-matched controls (n=26; top-80 +2.9)</div></div>
...
<p class="sub" style="max-width:100ch">Rebuilt strictly as of end-2025 (precedent, P(add), and weights restricted to ≤2025 data): the opportunity top-40 gained +3.1 Stuff+ in 2026 vs −0.2 for stuff-matched controls (n=20 with ≥300 pitches; top-80 +1.6). On the actionable score, Beck Way is #6 in 2026 (mix +8.5: 128 slider thrown 15%, 84 four-seam 14%; curve reachable at 103; drop-recipe profile) and the as-of-2025 top-40 gained +2.5 vs −0.6 matched. On drift, Palmquist ranked #27 (mix +5.4: threw his 86 four-seam 42%; pitch gap +9.2: FF 86 → comps 101, FC 75 → 93), Senzatela #26, Dollander #29, Hancock #101 (#1 by raw addition gain — card said curve; he dropped 8° and added a sweeper/cutter). Caveat: several top-ranked 2025 relievers had no 2026 sample, so the excess is measured among survivors.</p>

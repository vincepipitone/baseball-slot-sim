...
for _,r in d.iterrows():
    ...
    rows.append(dict(name=r.PlayerName,team=r.Team if isinstance(r.Team,str) else '',age=f(r.Age),role=r.role,ip=f(r.IP),slot=f(r.arm_angle),eff4=f(r.eff4,2),
        ...
        opp=f(r.opportunity,2),mix=f(r.gain_mix,1),gap=f(r.gain_gap,1),gapp=(r.gap_parts if isinstance(r.gap_parts,str) else ''),coors=f(r.coors_adj,1),colshare=f(r.col_share,2) if pd.notna(r.col_share) else None,worst=f'{r.worst_fam} {r.worst_stf:.0f} @ {r.worst_use*100:.0f}%' if isinstance(r.worst_fam,str) else '',bestp=f'{r.best_fam} {r.best_stf:.0f} @ {r.best_use*100:.0f}%' if isinstance(r.best_fam,str) else '',projall=f(r.proj_stuff_all),best=r.best_add,bpstf=f(r.best_add_pstf),bprec=f(r.best_add_prec,2),bp=f(r.best_add_padd,2),gain=f(r.gain,1),ev=f(r.sum_ev,2),reach=f(r.n_reachable),
...
let sortKey='opp',sortDir=-1,filt={q:'',role:'',cls:'',reach:false,drop:false};
const cols=[['name','Pitcher','l'],['team','Tm','l'],['age','Age'],['role','Role','l'],['ip','IP'],['slot','Slot°'],['eff4','4S eff'],['cls','Class','l'],['stuff','Stuff+'],['own','Own'],['loc','Loc+'],['opp','Opportunity'],['gap','Pitch gap'],['mix','Mix'],['ev','Add ΣEV'],['best','Best add','l'],['bpstf','Prec Stf+'],['bp','P(add)'],['coors','Coors'],['pool','Pool'],['projall','Proj Stf+'],['dwar','ΔWAR (add)']];
function fmt(v,k){if(v===null||v===undefined)return '<span style="color:var(--muted)">·</span>';if(k==='cls'){const g=CLSG[v]||'H';return `<span class="chip ${g}">${v.replace('_',' ')}</span>`}
 if(k==='gain'||k==='dwar'||k==='dm'||k==='gap'||k==='mix'||k==='coors'){const s=(v>0?'+':'')+v.toFixed(k==='dwar'?2:1);return v>0?`<span class="pos">${s}</span>`:v<0?`<span class="neg">${s}</span>`:s}
 if(k==='opp'){return `<span class="bar" style="width:${Math.min(70,v*14)}px"></span>${v.toFixed(2)}`}if(k==='ev')return v.toFixed(2);
...
render();
"""
ths=''.join(f'<th data-k="{k}" class="{c if len(x)>2 else ""}">{lab}</th>' for x in [['name','Pitcher','l'],['team','Tm','l'],['age','Age'],['role','Role','l'],['ip','IP'],['slot','Slot°'],['eff4','4S eff'],['cls','Class','l'],['stuff','Stuff+'],['own','Own'],['loc','Loc+'],['opp','Opportunity'],['gap','Pitch gap'],['mix','Mix'],['ev','Add ΣEV'],['best','Best add','l'],['bpstf','Prec Stf+'],['bp','P(add)'],['coors','Coors'],['pool','Pool'],['projall','Proj Stf+'],['dwar','ΔWAR (add)']] for k,lab,*c in [x] for c in [c[0] if c else ''])
def check(nm):
...
<div class="stat"><div class="v">+3.3</div><div class="l">2026 Stuff+ excess of the as-of-2025 opportunity top-40 vs stuff-matched controls (n=20; top-80 +1.6)</div></div>
<div class="stat"><div class="v">t = 5.9</div><div class="l">existing-pitch gap on next-season ΔStuff+ after mean reversion (2020–25, n=2274)</div></div>
</div>
<div class="method">
...
<p class="sub" style="max-width:100ch">Rebuilt strictly as of end-2025 (precedent, P(add), and weights restricted to ≤2025 data): the opportunity top-40 gained +3.1 Stuff+ in 2026 vs −0.2 for stuff-matched controls (n=20 with ≥300 pitches; top-80 +1.6). Palmquist ranked #27 (mix +5.4: threw his 86 four-seam 42%; pitch gap +9.2: FF 86 → comps 101, FC 75 → 93), Senzatela #26, Dollander #29, Hancock #101 (#1 by raw addition gain — card said curve; he dropped 8° and added a sweeper/cutter). Caveat: several top-ranked 2025 relievers had no 2026 sample, so the excess is measured among survivors.</p>

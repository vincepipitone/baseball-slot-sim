...
tr25=d5[(d5.game_year<=2024)&d5.d_stuff.notna()].dropna(subset=ARCH); te25=d5[d5.game_year==2025].dropna(subset=ARCH).copy(); te25['pred']=fitpred(tr25,te25,ARCH)
full=d5[d5.d_stuff.notna()].dropna(subset=ARCH); s26=d6.dropna(subset=ARCH).copy(); s26['pred']=fitpred(full,s26,ARCH)
...
def combine(df):
    df=df[gate(df)].copy(); z=lambda s:(s-s.mean())/s.std(); df['z_dev']=z(df.pred); df['z_act']=z(df.actionable.clip(lower=0)); df['cpm']=df.z_dev+df.z_act; df['rk']=df.cpm.rank(ascending=False,method='min'); return df
...
print(f"2025→26 gated CPM: top-10 excess ΔStuff+ {excess(g25,'cpm',10,'d_stuff','stuff'):+.2f} ΔPit+ {excess(g25,'cpm',10,'d_pit','sp_pitching'):+.2f} | top-25 {excess(g25,'cpm',25,'d_stuff','stuff'):+.2f}")
...
for nm in ['Palmquist','Hancock','Dollander','Kyle Harrison','Sasaki']:
    x=g25[g25.PlayerName.str.contains(nm,na=False)]; y=te25[te25.PlayerName.str.contains(nm,na=False)]
    print(f"  2025 {nm}: gated CPM {'#%.0f/%d'%(x.rk.iloc[0],len(g25)) if len(x) else 'out of gate'} | ungated dev {'#%.0f/%d (%+.1f cal, realized %+.1f)'%(y.pred.rank(ascending=False)[y.index[0]],len(te25),y.pred_cal.iloc[0],y.d_stuff.iloc[0]) if len(y) else '-'}")
for nm in ['Beck Way','Mason Black','Kyle Harrison']:
    x=g26[g26.PlayerName.str.contains(nm,na=False)]; y=s26[s26.PlayerName.str.contains(nm,na=False)]
    print(f"  2026 {nm}: gated CPM {'#%.0f/%d'%(x.rk.iloc[0],len(g26)) if len(x) else 'out of gate'} | ungated dev {'#%.0f/%d (%+.1f cal)'%(y.pred.rank(ascending=False)[y.index[0]],len(s26),y.pred_cal.iloc[0]) if len(y) else '-'}")
